...
from ethjsonrpc.errors import JsonRPCError, HTTPError
from ethjsonrpc.utils import parse_int, validate_hex_int, validate_block_param

JSONRPC_LOG = logging.getLogger("ethjsonrpc.client")

# select which client to use
try:
    # if aiohttp is available, prefer that
    import aiohttp
    from ethjsonrpc.aiohttp_client import HTTPClient
    JSONRPC_LOG.debug("using aiohttp jsonrpc client")
except:
    try:
        from ethjsonrpc.tornado_client import HTTPClient
...
class JsonRPCClient:
    ...
    def __init__(self, url, *, websocket_url=None, should_retry=True, log=None,
